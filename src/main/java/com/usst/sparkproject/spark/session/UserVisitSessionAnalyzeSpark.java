...
import com.usst.sparkproject.domain.SessionDetail;
import com.usst.sparkproject.domain.SessionRandomExtract;
...
import com.usst.sparkproject.util.StringUtils;
import com.usst.sparkproject.util.ValidUtils;
...
/*				.setMaster("local")
		...
		System.out.println("taskid= " + args[0]);
		...
		JavaPairRDD<String, Row> sessionid2actionRDD = getSessionid2ActionRDD(actionRDD); // <sessonId,row>
		...
		JavaPairRDD<String, String> sessionid2AggrInfoRDD = aggregateBySession(sc, sqlContext, actionRDD);
		 ...
		 * 
		 * 开发Spark大型复杂项目的一些经验准则： 1、尽量少生成RDD 2、尽量少对RDD进行算子操作，如果有可能，尽量在一个算子里面，实现多个需要做的功能
	...
	/**
	 * 过滤session数据 如果 param的参数是空的，就是说过滤数据的条件 全部都符合，方法返回ture，取反为false，
	 * 
...
//				+ "and session_id not in('','','')"
	 ...
	 *         到这里为止，获取的数据是<sessionid,(sessionid,searchKeywords,clickCategoryIds,age,professional,city,sex)>
	 */
	private static JavaPairRDD<String, String> aggregateBySession1(JavaSparkContext sc, SQLContext sqlContext,
...
//						});
	...
	/**
	 * 获取SQLContext 如果是在本地测试环境的话，那么就生成SQLContext对象 如果是在生产环境运行的话，那么就生成HiveContext对象
	 * 
...
//		});
	...
	private static void randomExtractSession(final long taskid, JavaPairRDD<String, String> sessionid2AggrInfoRDD,
		...
		/**
		 * 第一步：<sessionID,aggrInfo> 转为一条一条的 <dateHour, aggrIno>
		 */
		JavaPairRDD<String, String> time2sessionidRDD = sessionid2AggrInfoRDD
				.mapToPair(new PairFunction<Tuple2<String, String>, String, String>() {
					...
					public Tuple2<String, String> call(Tuple2<String, String> tuple) throws Exception {
						...
						return new Tuple2<String, String>(hour, aggrInfo);
					}
				});
		...
		// 得到每天每小时的session数量,格式是<key, key出现的次数，此时object的类型适用于long或者integer>
		Map<String, Object> countMap = time2sessionidRDD.countByKey();

		/**
		 * 第二部：使用按时间比例随机抽取算法，计算出每天每小时要抽取的session的索引
		 * 
		 */

		// 将<yyyy-dd-dd_hh, count> 格式的map。转化成<yyyy-mm-dd, <HH ,count>>的格式
		...
		// 转化<yyyy-mm-dd, <HH ,count>> 转换成 <date,<hour,(3,5,20,102)>>
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(long taskid,
		...
		// 获取符合条件的session的访问明细,sessionid2actionRDD是所有的session，filteredSessionid2AggrInfoRDD是过滤后符合的sessionId组成的tuple
		/*
		 * JavaPairRDD<String, Row> sessionid2detailRDD =
		 * filteredSessionid2AggrInfoRDD.join(sessionid2actionRDD) .mapToPair(new
		 * PairFunction<Tuple2<String,Tuple2<String,Row>>, String, Row>() {
		 * 
		 * private static final long serialVersionUID = 1L;
		 * 
		 * @Override public Tuple2<String, Row> call(Tuple2<String, Tuple2<String, Row>>
		 * tuple) throws Exception { return new Tuple2<String, Row>(tuple._1,
		 * tuple._2._2); } });
		 */

		// 获取session访问过的所有品类id
		// 访问过：指的是，点击过、下单过、支付过的品类 返回的格式是：<orderCategoryIds,orderCategoryIds>
		// 或者<clickCategoryId,clickCategoryId> <payCategoryIds,payCategoryIds>
		 ...
		 * join到自己的某个数据，比如点击、或下单、或支付次数，那么该categoryidRDD还是要保留下来的 只不过，没有join到的那个数据，就是0了
		 * 
		 */
